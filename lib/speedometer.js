Speedometer = function(spec) {
	...
	function drawSpeedometerColourArc(options) {
		...
		options.ctx.arc(options.center.X,
			options.center.Y,
			options.radius - 10, (2 * Math.PI / 360) * 60, (2 * Math.PI / 360) * 270,
			true);
		options.ctx.stroke();
		options.ctx.beginPath();
		...
		options.ctx.arc(options.center.X,
			options.center.Y,
			options.radius - 10, (2 * Math.PI / 360) * 270, (2 * Math.PI / 360) * 120,
			true);
		options.ctx.stroke();
	}
	...
	function drawScore(options) {
		...
		if (spec.maxVal === 0) {
			options.ctx.fillText('-', fromX + width / 2, fromY + height / 6);
		} else {
